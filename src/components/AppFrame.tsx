      ...
      <header className="sticky top-0 z-40 bg-[#3D2B0E] text-white" style={{ paddingTop: "env(safe-area-inset-top)" }}>
        <div className="max-w-lg mx-auto flex items-center justify-between px-4 h-14">
          <Link to="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="font-extrabold tracking-tight text-[15px]">Van-Link</span>
          </Link>

          <div className="flex items-center gap-1">
            {user && (
              <Link to="/support" className="relative h-9 w-9 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors">
            ...
            {user ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  {isDriver && <DropdownMenuItem asChild><Link to="/my-vehicle">My vehicle</Link></DropdownMenuItem>}
                  ...
                  {isAdmin && <DropdownMenuItem asChild><Link to="/analytics">Analytics</Link></DropdownMenuItem>}
                  <DropdownMenuSeparator />
              ...
              <Link to="/login" className="text-sm font-bold px-3 h-9 flex items-center rounded-lg bg-[#C9A05A] text-[#3D2B0E]">
                Sign in
              </Link>
            )}
          </div>
          ...
          <div className="max-w-lg mx-auto grid grid-cols-4">
            {tabs.map(t => {
              ...
              return (
                ...
                >
                  <Icon className="h-5 w-5" style={{ color: active ? "#3D2B0E" : "#B08A45" }} strokeWidth={active ? 2.4 : 2} />
                  <span className="text-[10px] font-bold" style={{ color: active ? "#3D2B0E" : "#B08A45" }}>{t.label}</span>
                </Link>
              );
            })}
          </div>
