...
export default function BookingCard({ booking, onClick }: any) {
  ...
  const body = (
    <div className="bg-white rounded-2xl p-4 border border-[#E5E7EB] hover:border-[#C9A05A]/40 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11px] font-bold px-2 py-1 rounded-full" style={{ color, background: color + "1A" }}>
          {getStatusLabel(booking.status)}
        </span>
        <span className="text-sm font-extrabold text-[#3D2B0E] tabular-nums">P{booking.offer}</span>
      </div>
      <div className="space-y-1">
        <p className="flex items-center gap-1.5 text-sm font-semibold text-[#3D2B0E]">
