    ...
    <div className="space-y-2">
      {rows.map(b => (
        <div key={b.id} className="bg-white border border-[#E5E7EB] rounded-2xl p-4 flex items-start gap-3">
          <div className="h-9 w-9 rounded-xl bg-[#FFF0E6] flex items-center justify-center shrink-0">
            <Calendar className="h-4 w-4 text-[#F97316]" />
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-bold text-[#0F0F0F] text-sm truncate">{b.pickup} → {b.dropoff}</p>
